...
class YoLov5TRT(object):
    ...
    def infer(self, input_image, cls='person'):
        ...
        for i in range(len(result_boxes)):
            if result_classid[i]==categories.index(cls):
                box = result_boxes[i]
                plot_one_box(
                    box,
                    image_raw,
...
if __name__ == "__main__":
    ...
    cam=cv2.VideoCapture('/home/roboi1/Downloads/no_distortion2.avi')
    #cam.set(3,640)
    #cam.set(4,480)
    ...
    while True:
        ...
        if _ :
            yolov5_wrapper.infer(img,cls='dog')
        else :
            break

        dt=time()-t
        print('fps : {0:.2f}'.format(1/dt))
        if cv2.waitKey(7)==27:
            cam.release()
